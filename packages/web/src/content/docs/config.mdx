...
7. **Managed config** (system directory) - admin-controlled, enterprise only

This means project configs can override global defaults, and global configs can override remote organizational defaults.

:::tip
For enterprise deployments, managed configuration (item 7) has the highest priority and cannot be overridden by users. [Learn more about managed configuration](/docs/enterprise#managed-configuration).
:::
...
For enterprise deployments, administrators can place configuration files in system directories that override all user settings. This is useful for enforcing organization-wide policies and security restrictions.

Managed config is loaded last with the highest priority and cannot be overridden by users.

[Learn more about managed configuration in the enterprise documentation](/docs/enterprise#managed-configuration).
